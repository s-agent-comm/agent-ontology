...
We adopt an "Ontology as a Single Source of Truth" development model. All manual modifications should only target the ontologies (`.ttl`) or their validation rules (`.shacl.ttl`). Generated serializations live in `dist/`. Contexts and profiles are curated alongside the ontology and should be updated when new terms are introduced.
...
### Step 2: Automatically Generate JSON-LD Serializations

After modifying the ontology, run the following script to update the auto-generated JSON-LD files in `dist/`.

```bash
python tools/generate_jsonld.py
```

This step ensures that the JSON-LD serializations are synchronized with the ontology definitions. If you introduce new terms, update `context/` and `profiles/` as needed to keep them aligned.
